...
def visualize_waveform(sr: int, duration: float = 0.5) -> np.ndarray:
    """
    파형 시각화 (실습 1)

    파형(Waveform)이란?
    - 소리는 공기의 압력 변화 = 파동
    - 이 파동을 시간에 따라 기록한 것이 파형
    - 가로축: 시간 (초)
    - 세로축: 진폭 (amplitude) - 소리의 크기

    샘플링(Sampling)이란?
    - 연속적인 소리를 디지털로 저장하려면 이산적인 값으로 변환 필요
    - 샘플링 레이트(sr): 1초에 몇 번 측정하는가 (예: 22050Hz = 초당 22050번)
    - 나이퀴스트 정리: sr의 절반 주파수까지만 표현 가능 (22050Hz -> 11025Hz 까지)
    
    합성 오디오 구성:
    - 기본음(fundamental): 440Hz (A4 음)
    - 배음(harmonics): 880Hz, 1320Hz, 1760Hz (정수배 주파수)
    - 배음이 있어야 악기처럼 풍부한 음색 표현 가능

    Args:
        sr: 샘플링 레이트 (Hz). 기본값 22050.
        duration: 신호 길이 (초). 기본값 0.5.

    Returns:
        생성된 합성 오디오 신호 (1D numpy array)
    """
    print("\n[1] 파형 시각화")
    ...
    plt.tight_layout()
    plt.savefig('outputs/01_synthetic_waveform.png', dpi=150)
    plt.close()
...
def compare_sine_waves(sr: int, duration: float = 0.05) -> None:
    """
    사인파 합성 비교 (실습 2)

    사인파(Sine Wave)란?
    - 가장 단순한 파동의 형태: y = A * sin(2 * pi * f * t)
    - A: 진폭 (amplitude) - 소리 크기
    - f: 주파수 (frequency) - 음높이 (Hz)
    - t: 시간 (초)

    단일 사인파 vs 복합파:
    - 단일 사인파: 하나의 주파수만 포함 (순음, pure tone)
    - 복합파: 여러 주파수의 합성 (악기 소리처럼 풍부한 음색)

    배음(Harmonics)이란?
    - 기본음(fundamental)의 정수배 주파수
    - 440Hz 기본음 -> 880Hz(2배음), 1320Hz(3배음), ...
    - 배음의 조합이 악기마다 다른 음색(timbre)을 만듦
    - 피아노, 바이올린, 트럼펫이 같은 음을 쳐도 다르게 들리는 이유

    Args:
        sr: 샘플링 레이트
        duration: 시각화용 신호 길이 (초)
    """
    print("\n[2] 사인파 합성")
    ...
    axes[0].plot(t * 1000, sine_single, linewidth=0.8)
...
def analyze_fft(sr: int, duration: float = 0.5) -> None:
    """
    FFT 분석 (실습 3)

    FFT(Fast Fourier Transform)란?
    - 시간 영역 신호를 주파수 영역으로 변환하는 알고리즘
    - 입력: 시간에 따른 진폭 값들 (파형)
    - 출력: 각 주파수 성분의 크기와 위상

    FFT가 알려주는 것:
    - 신호에 어떤 주파수가 포함되어 있는가
    - 각 주파수의 상대적 세기는 얼마인가
    - 예: 440Hz 피크 = 이 소리에 440Hz 성분이 강하게 존재

    FFT의 한계:
    - 전체 신호를 한 번에 분석 -> "언제" 그 주파수가 있었는지 알 수 없음
    - 이 문제를 해결하는 것이 STFT (다음 실습에서 다룸)

    Args:
        sr: 샘플링 레이트 (Hz)
        duration: 분석용 신호 길이 (초). 기본값 0.5.
    """
    print("\n[3] FFT 분석")

    # 청취용 오디오 생성 및 저장 (1초)
    audio_for_listen = generate_synthetic_audio(sr, duration=1.0)
    ...
    magnitude = magnitude / np.max(magnitude)  # 정규화
...
def analyze_trumpet(sr: int) -> None:
    """
    실제 악기 소리 분석 (실습 4)

    합성 오디오 vs 실제 악기:
    - 합성 오디오: 수학적으로 정의된 순수한 사인파 조합
    - 실제 악기: 물리적 진동으로 인한 복잡한 파형

    실제 악기의 특징:
    - FFT 결과가 날카로운 피크가 아닌 넓은 분포
    - 시간에 따라 음색(timbre)이 변화 (Attack, Sustain, Release)
    - 노이즈 성분 포함 (숨소리, 마찰음 등)
    ...
    trumpet, _ = librosa.load(trumpet_path, sr=sr)
    ...
    print(f"    트럼펫 샘플: {len(trumpet)} 샘플 ({len(trumpet)/sr:.1f}초)")
...
def main():
    ...
    # 실습 1: 파형 시각화
    visualize_waveform(sr)

    # 실습 2: 사인파 합성
    compare_sine_waves(sr)
